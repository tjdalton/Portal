...
public class PortalPlayerListener extends PlayerListener {
	...
	public void onPlayerMove(PlayerMoveEvent event) {
		...
		if ((loc.getWorld().getBlockTypeIdAt(loc.getBlockX(),
			...
			if ((loc.getWorld().getBlockTypeIdAt(loc.getBlockX(),
					loc.getBlockY(), loc.getBlockZ()) == Material.PORTAL.getId())) {
				...
				if (!(((newLoc.getWorld().getBlockTypeIdAt(newLoc.getBlockX(),
						newLoc.getBlockY() - 1, newLoc.getBlockZ()) == Material.OBSIDIAN.getId()) && (newLoc
						.getWorld().getBlockTypeIdAt(newLoc.getBlockX() - 1,
								newLoc.getBlockY() - 1, newLoc.getBlockZ()) == Material.OBSIDIAN.getId())) || ((newLoc
						.getWorld().getBlockTypeIdAt(newLoc.getBlockX() + 1,
								newLoc.getBlockY() - 1, newLoc.getBlockZ()) == Material.OBSIDIAN.getId())
						&& (newLoc.getWorld().getBlockTypeIdAt(
								newLoc.getBlockX(), newLoc.getBlockY() - 1,
								newLoc.getBlockZ()) == Material.OBSIDIAN.getId())
						|| ((newLoc.getWorld().getBlockTypeIdAt(
								newLoc.getBlockX(), newLoc.getBlockY() - 1,
								newLoc.getBlockZ() + 1) == Material.OBSIDIAN.getId()) && (newLoc
								.getWorld().getBlockTypeIdAt(
										newLoc.getBlockX(),
										newLoc.getBlockY() - 1,
										newLoc.getBlockZ()) == Material.OBSIDIAN.getId())) || ((newLoc
						.getWorld().getBlockTypeIdAt(newLoc.getBlockX(),
								newLoc.getBlockY() - 1, newLoc.getBlockZ() - 1) == Material.OBSIDIAN.getId()) && (newLoc
						.getWorld().getBlockTypeIdAt(newLoc.getBlockX(),
								newLoc.getBlockY() - 1, newLoc.getBlockZ()) == Material.OBSIDIAN.getId()))))) {
					...
					while (newLoc.getWorld().getBlockTypeIdAt(
							...
							|| newLoc.getWorld().getBlockTypeIdAt(
									newLoc.getBlockX(), newLoc.getBlockY() - 1,
									newLoc.getBlockZ()) == Material.PORTAL.getId()) {
					...
					while (newLoc.getWorld().getBlockTypeIdAt(
							...
							&& newLoc.getWorld().getBlockTypeIdAt(
									newLoc.getBlockX() + 2,
									newLoc.getBlockY() + 1, newLoc.getBlockZ()) != Material.AIR.getId()) {
					...
					for (int i = 0; i < 5; i++) {
						for (int j = 1; j < 2; j++) {
							// for (int k = -2; k < 2; k++) {
							world.getBlockAt(newLoc.getBlockX() + i,
									newLoc.getBlockY() + j, newLoc.getBlockZ())
						...
						for (int j = -20; j <= 20; j++) {
							for (int k = -20; k <= 20; k++) {
								if (newLoc.getWorld().getBlockTypeIdAt(
										newLoc.getBlockX() + j,
										newLoc.getBlockY() + i,
										newLoc.getBlockZ() + k) == Material.OBSIDIAN.getId()
										|| newLoc.getWorld().getBlockTypeIdAt(
												newLoc.getBlockX() + j,
												newLoc.getBlockY() + i,
												newLoc.getBlockZ() + k) == Material.PORTAL.getId()) {
				...
				// Adjust destination to prevent clipping and trigger teleport.
